...
function timeSpanToString(startDate, endDate) {
  ...
  let millSec = endDate.getMilliseconds() - startDate.getMilliseconds();
  if (min < 10) {
  ...
  if (millSec < 10) {
    millSec = String(millSec).padStart(3, '0');
  }
  return `0${hour}:${min}:${sec}.${millSec}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const degreeHour = 0.5 * (60 * hour + min);
  const degreeMin = 6 * min;
  let degree = degreeHour - degreeMin;
  if (degree > 180) {
    degree -= 180;
  }
